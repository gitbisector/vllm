...
class DeepseekV4MegaMoEExperts(nn.Module):
    ...
    def _finalize_shared_expert_weights(
        ...
        # MERGE-FLAG (2026-09-03, dsv4-b12x-rebase): took upstream's side
        # here -- it adds real dtype/shape validation with warnings that
        # HEAD's version lacked entirely (HEAD fell straight through to a
        # transform call with no checks). Not fully certain HEAD's original
        # method boundary here matched upstream's (finalize_mega_moe_weights
        # vs finalize_weights) -- flows correctly into the shared continuation
        # either way, but worth a second look before trusting MegaMoE shared-
        # expert weight loading blindly.
