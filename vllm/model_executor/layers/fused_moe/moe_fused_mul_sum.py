...
def moe_fused_mul_sum_kernel(
    ...
    if has_topk_ids:
        ...
        for n in tl.static_range(top_k):
            ...
            if has_expert_map:
                ...
                # the length of the map actually being indexed -- not just
                # id_val >= 0. See 2237d87af0 (vllm-project/vllm#41834):
                # unbounded ids here is an illegal-memory-access that takes
                # down every TP rank at once.
                id_in_range = valid & (id_val < num_global_experts)
                safe_id = tl.where(id_in_range, id_val, 0)
                take = id_in_range & (tl.load(expert_map_ptr + safe_id) >= 0)
    ...
    # MERGE-FLAG (2026-09-03, dsv4-b12x-rebase): upstream rewrote this kernel
    # from a BLOCK_M-of-tokens-per-CTA design (HEAD's side, dropped here) to
    # one-CTA-per-token with precomputed takes/weights. Kept upstream's
    # structure since HEAD's body referenced offs_m/mask/size/BLOCK_M that no
    # longer exist anywhere in the merged preamble -- it was not reconcilable
    # in place. jasl's only real change here (2237d87af0, the expert-count
    # bound) is preserved above in the takes-computation loop instead.
