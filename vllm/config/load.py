...
class LoadConfig:
    ...
    - "instanttensor" will load the Safetensors weights on CUDA devices using
      ...
      and fast direct I/O.
    - "fastsafetensors" will load the Safetensors weights directly onto the GPU
      using the fastsafetensors library, via GPUDirect Storage where it is
      available. See the fastsafetensors page in the Extensions section for more
      information.
