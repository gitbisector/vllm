...
class VllmConfig:
    ...
    def _uses_breakable_cudagraph_by_default(self) -> bool:
        ...
        # DeepSeek-V4 is deliberately excluded: breakable mode disables the
        # torch.compile pipeline (-cc.mode=none) and runs attention eagerly
        # every decode step, which on SM12x is 1.5-3.8x SLOWER for MTP decode
        # and degrades with output length (measured on RTX PRO 6000/SM120 and
        # 2x GB10/SM121). FULL_AND_PIECEWISE + torch.compile is faster and
        # GSM8K-parity correct, so it stays the DSv4 default. Opt in with
        # VLLM_USE_BREAKABLE_CUDAGRAPH=1.
        eligible = default_breakable_cudagraph_architectures() - {
            "DeepseekV4ForCausalLM",
            "DeepSeekV4MTPModel",
        }
        return bool(architectures & eligible)
    ...
    def __post_init__(self):
        ...
        # MERGE-NOTE (2026-09-03, dsv4-b12x-rebase): adopted upstream's
        # refactor (_maybe_enable_breakable_cudagraph method) over this fork's
        # inline duplicate; the DeepSeek-V4 exclusion now lives inside
        # _uses_breakable_cudagraph_by_default's carve-out instead of a
        # separate _should_auto_enable_breakable_cudagraph gate at this call
        # site. That old function/set is kept for
        # tests/config/test_deepseek_v4_cudagraph_config.py, which imports it
        # directly, but is no longer consulted here -- verify the two don't
        # need reconciling further if that test's behavior matters post-merge.
