    ...
    def _dummy_run(
        ...
        **kwargs,
        ...
        if self.speculator is not None:
            ...
            # MERGE-NOTE (2026-09-03): kept HEAD's "is not None" guard (dropped
            # upstream, which instead added a `# type: ignore[union-attr]` --
            # a sign upstream's own type checker also flagged this as
            # possibly-None) and combined it with upstream's new
            # use_workspace_lane wrapping, which HEAD didn't have.
            with use_workspace_lane(self._draft_workspace_lane):
