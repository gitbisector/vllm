...
class DeepseekV32IndexerMetadataBuilder(AttentionMetadataBuilder):
    ...
    def build(
        ...
        if num_decodes > 0:
            ...
            # MERGE-NOTE (2026-09-03): kept jasl's _uses_deep_gemm_scheduler_metadata()
            # gate (excludes family(120)/SM12x) over upstream's plain is_cuda()+
            # has_deep_gemm() check -- upstream's version would re-enable this
            # DeepGEMM path on GB10, which the gate was deliberately written to
